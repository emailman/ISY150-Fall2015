...
def main():
    ...
    Label(window, text="Annual Interest Rate").grid(row=1, column=0, sticky=W)
    Label(window, text="Number of Years").grid(row=2, column=0, sticky=W)
    Label(window, text="Loan Payment").grid(row=3, column=0, sticky=W)
    Label(window, text="Monthly Payment", fg="BLUE").grid(row=4, column=0, sticky=W)
    Label(window, text="Total Payment", fg="BLUE").grid(row=5, column=0, sticky=W)

    # Create text fields in column 1
    global annual_interest_rate
    annual_interest_rate = StringVar()
    Entry(window, textvariable=annual_interest_rate, justify=RIGHT).grid(row=1, column=1)

    global number_of_years
    number_of_years = StringVar()
    Entry(window, textvariable=number_of_years, justify=RIGHT).grid(row=2, column=1)

    global loan_amount
    loan_amount = StringVar()
    Entry(window, textvariable=loan_amount, justify=RIGHT).grid(row=3, column=1)

    global monthly_payment
    monthly_payment = StringVar()
    Entry(window, textvariable=monthly_payment, justify=RIGHT, state="disabled").grid(row=4, column=1)

    global total_payment
    total_payment = StringVar()
    Entry(window, textvariable=total_payment, justify=RIGHT, state="disabled").grid(row=5, column=1)

    # Create two buttons in row 6
    Button(window, bg="RED", text="Quit",
